...
__host__
int run_split_orig(const float* img, int* seg, bool* border,
  ...
  if(idx%4 == 0){
    ...
    // -- run split --
    max_spix = CudaCalcSplitCandidate_orig(img, seg, border,
...
__host__
void run_merge_orig(const float* img, int* seg, bool* border,
               spix_params* sp_params, spix_helper* sp_helper,
               spix_helper_sm* sm_helper,
               int* sm_seg1, int* sm_seg2, int* sm_pairs,
  ...
  if( idx%4 == 2){
    ...
    CudaCalcMergeCandidate_orig(img, seg, border,
                                sp_params, sp_helper, sm_helper, sm_pairs,
                                sp_size,npix,nbatch,width,height,nftrs,
                                nspix_buffer,direction,
                                alpha_hastings,alpha_merge,
                                sigma2_app, sigma2_size, logger);

  }
}

__host__ void CudaCalcMergeCandidate_orig(const float* img, int* seg, bool* border,
                                     spix_params* sp_params,spix_helper* sp_helper,
    ...
    calc_merge_stats_step0<<<BlockPerGrid2,ThreadPerBlock>>>(sm_pairs,sp_params,
                                                             sp_helper,sm_helper,
                                                             nspix_buffer, b_0);
    // gpuErrchk( cudaPeekAtLastError() );
    // gpuErrchk( cudaDeviceSynchronize() );

    calc_merge_stats_step1<<<BlockPerGrid2,ThreadPerBlock>>>(sp_params,sm_helper,
                                                             nspix_buffer,a_0, b_0);
    // gpuErrchk( cudaPeekAtLastError() );
    // gpuErrchk( cudaDeviceSynchronize() );

    calc_merge_stats_step2<<<BlockPerGrid2,ThreadPerBlock>>>(sm_pairs,sp_params,
    ...
    update_merge_flag<<<BlockPerGrid2,ThreadPerBlock>>>(sm_pairs,sp_params,
                                                        sm_helper,nspix_buffer,
                                                        nmerges_gpu);
...
__host__ int CudaCalcSplitCandidate_orig(const float* img, int* seg, bool* border,
    ...
    // -- summary stats --
    calc_split_stats_step0<<<BlockPerGrid2,ThreadPerBlock>>>(sp_params, sp_helper,
                                                             sm_helper, nspix_buffer,
                                                             b_0, max_spix);
    calc_split_stats_step1<<<BlockPerGrid2,ThreadPerBlock>>>(sp_params, sp_helper,
                                                             sm_helper, nspix_buffer,
                                                             a_0, b_0, max_spix);

    // -- update the flag using hastings ratio --
    update_split_flag<<<BlockPerGrid2,ThreadPerBlock>>>(sm_pairs, sp_params,
                                                        sm_helper,nspix_buffer,
                                                        alpha, split_alpha,
                                                        max_spix, max_sp);
...
__global__ void calc_merge_stats_step0(int* sm_pairs,
                                       spix_params* sp_params,
                                       spix_helper* sp_helper,
                                       spix_helper_sm* sm_helper,
...
__global__
void calc_merge_stats_step1(spix_params* sp_params,spix_helper_sm* sm_helper,
...
__global__ void calc_merge_stats_step2(int* sm_pairs,
                                       spix_params* sp_params,
                                       spix_helper_sm* sm_helper,
                                       const int nspix_buffer,
                                       float alpha, float alpha_merge) {
	// getting the index of the pixel
	int k = threadIdx.x + blockIdx.x * blockDim.x;  // the label
   ...
   float log_denominator = __logf(alpha) + lgammaf(count_k) + lgammaf(count_f -  count_k) + total_marginal_1 + 
    ...
    sm_helper[k].hasting = log_nominator - log_denominator + alpha_merge;
...
void update_split_flag(int* sm_pairs,
                       ...
                       spix_helper_sm* sm_helper,
                       const int nspix_buffer,
                       float alpha, float split_alpha, int max_spix, int* max_sp ) {
